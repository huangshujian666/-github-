...
config_custom = {
    ...
    "factor_dict": [
        {
            "func_name_factor": "commodity_cross_variety_arbitrage_signal",
            "param_dataSrc": [
                {
                    "func_name_dataSrc": "query_DB",
                    "colName_dataSrc": [
                    ...
                    # 使用具体交割月合约。旧版账户模块可识别此格式并生成订单日志。
                    "security": ["RB1809.SHF"],
                    "colName_tradePrice": "open",
                },
                {
                    "func_name_dataSrc": "query_DB",
                    "colName_dataSrc": [
            ...
            "param_factor": {
                ...
                "exit_sigma": 0.0,
                "stop_loss_sigma": 3.0,
                "candidate_pairs": [("RB1809.SHF", "HC1809.SHF")],
                # 该合约对在此历史区间通过候选期的统计筛选；交易从筛选期结束后开始。
                "correlation_start": "2017-09-18",
                "correlation_end": "2018-03-30",
                "adf_p_value_threshold": 0.01,
                "engle_granger_p_value_threshold": 0.01,
                "execution_delay": 1,
                # 研报仅要求候选期通过协整筛选，交易期不以每日60日协整结果强制空仓。
                # 如需启用更严格的扩展风控模式，可将该值改为 True。
                "require_rolling_cointegration": False,
            },
            "file_name_model": "factor_arbitrage",

            # 筛选期结束后开始交易，并在进入交割月前结束回测。
            "start_date": "2018-04-02",
            "end_date": "2018-08-31",

        # 不修改 pipeline/dataGet 的前提下，必须使用账户模块可识别的具体合约代码；
            # 连续代码 RB.SHF、HC.SHF 只能生成信号，无法进入旧版账户模块的下单分支。

            # RB、HC 同属期货类别，只需要多头和空头两个方向子账户。
            # 覆盖通用配置中的 12 个子账户，减少无效账户遍历。
            "subportfolio_num": 2,
        },
